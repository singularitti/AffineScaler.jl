...
!!! info
    The style guide is always a work in progress, and not all AffineScaler code
    follows the rules. When modifying AffineScaler, please fix the style violations
...
AffineScaler uses [JuliaFormatter](https://github.com/domluna/JuliaFormatter.jl) as
an auto-formatting tool.

We use the options contained in [`.JuliaFormatter.toml`](https://github.com/singularitti/AffineScaler.jl/blob/main/.JuliaFormatter.toml).

To format your code, `cd` to the AffineScaler directory, then run:
...
!!! info
    A continuous integration check verifies that all PRs made to AffineScaler have
