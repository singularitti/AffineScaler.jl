...
[AffineScaler](https://github.com/singularitti/AffineScaler.jl).
...
2. Run the following commands and wait for them to finish:
   ...
   julia> Pkg.add("AffineScaler")
   ```

3. Run

   ```julia-repl
   julia> using AffineScaler
   ```
...
version of AffineScaler, type

```@repl
using Pkg
Pkg.update()
pkg"add https://github.com/singularitti/AffineScaler.jl"
```
...
our [GitHub repository](https://github.com/singularitti/AffineScaler.jl)
for new releases.
Once we release a new version, you can update AffineScaler by typing

```@repl
using Pkg
Pkg.update("AffineScaler")
Pkg.gc()
```
...
1. To uninstall, in a Julia session, run
   ...
   julia> Pkg.rm("AffineScaler")

   julia> Pkg.gc()
   ```

2. Press `Ctrl+D` to quit the current session. Start a new Julia session and
   reinstall AffineScaler.
